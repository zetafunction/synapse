...
pub struct WorkerHandle<I, O> {
    pub tx: flume::Sender<I>,
    pub rx: amy::Receiver<O>,
}

pub struct Worker<I, O> {
    pub rx: flume::Receiver<I>,
    pub tx: amy::Sender<O>,
}

impl<I: Debug + Send + 'static, O: Debug + Send + 'static> Worker<I, O> {
    /// `reg` is the controller's registrar. This will eventually be removed, but for now, only the
    /// worker threads themselves use the async runtime.
    pub fn new(reg: &mut amy::Registrar) -> io::Result<(WorkerHandle<I, O>, Self)> {
        let (ctx, crx) = reg.channel::<O>()?;
        let (wtx, wrx) = flume::unbounded::<I>();
...
mod tests {
    ...
    fn ping_pong_worker() {
        ...
        worker_handle.tx.send(Ping {}).unwrap();
        let join_handle = worker
            .run("worker", async |worker| {
                let Ok(_ping) = worker.rx.recv_async().await else {
